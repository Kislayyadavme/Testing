...
BOT_USERNAME = "YourBotUsername" # IMPORTANT: Update this (e.g., @MyBotName)
...
# --- HELPER FUNCTION FOR MARKDOWN SAFETY ---

def _escape_markdown(text: str) -> str:
    """
    Escapes characters that have special meaning in Telegram Markdown V1.
    This is necessary for inserting dynamic user data (like usernames) into 
    Markdown strings without breaking the formatting.
    """
    # Characters to escape: _ * ` [
    # Telegram's Markdown V1 mostly breaks on these four if they appear in text 
    # that is not explicitly wrapped in an inline code block (`...`).
    # For maximum safety when inserting into **bold** or *italic* blocks, 
    # we explicitly escape them.
    escape_chars = r'_*`['
    ...
    def get_user(self, user_id: int, username: Optional[str] = None, init: bool = False) -> Dict[str, Any]:
        ...
        if user_id not in self._users or init:
            self._users[user_id] = {
                ...
                'blocked': False,
                'has_started': False,
                'last_reward_ref_count': 0 # Added for reward tracking
            }
...
def _get_admin_user_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    for uid, user_data in sorted_users[:20]:
        ...
        # Use inline code block for name/ID to prevent Markdown errors from names like 'user_name'
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    user_id = user.id
    # Use full_name for a less error-prone display, and ensure username is available
    username = user.username or user.first_name
    
    user_data = DB.get_user(user_id, username=username)
    
    # ---------------------------------------------
    # IMPORTANT FIX: Escape the username for Markdown safety
    ...
    if context.args and user_id != ADMIN_CHAT_ID: # Prevent admin self-referral
        try:
            ...
            if DB.add_referral(referrer_id, user_id):
                ...
                referrer_name = _escape_markdown(referrer_data.get('username', f"User {referrer_id}")) # Escape referrer name
                
                # Notify the referrer - using inline code block for name/ID for safety
            ...
            logger.warning(f"Invalid referral ID in start payload or referrer blocked: {context.args[0]} or Markdown error in notification.")
    ...
    if not user_data['has_started']:
        ...
        welcome_text = (
            # Using the escaped username here
            f"💖 **HELLO, {escaped_username.upper()}!** 💖\n\n"
        ...
        welcome_text = (
            # Using the escaped username here
            f"🌟 **Welcome Back, {escaped_username}!** 🌟\n\n"
    ...
    final_text = f"{welcome_text}{referral_check_message}"
    
    # 4. FINAL SAFETY CATCH: Try to send with Markdown, fall back to plain text if it fails
    try:
        ...
        logger.error(f"Markdown failed for start message for user {user_id}. Falling back to plain text. Error: {e}")
        await update.message.reply_text(
            "💖 HELLO! Welcome to the Advanced Bot Hub! To unlock content, please join the channels below and verify your membership.",
            reply_markup=reply_markup,
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 4. Admin Management Flow
        ...
        target_name = _escape_markdown(DB.get_user(target_id).get('username', f'User {target_id}'))
        
        # Use inline code block for target name/ID to prevent Markdown parsing issues
        await query.edit_message_text(
            f"**Manage User:** `{target_name}` **(ID: {target_id})**",
...
async def _handle_admin_request(query: Update.callback_query, context: ContextTypes.DEFAULT_TYPE, requester_id: int, original_requester_id: int) -> None:
    ...
    requester_name = _escape_markdown(query.from_user.full_name) # Escaping name for notification
    ...
    # Use inline code block for username and ID to prevent Markdown parsing issues
    admin_message = (
...
async def _handle_admin_decision(query: Update.callback_query, context: ContextTypes.DEFAULT_TYPE, data: str) -> None:
    ...
    target_data = DB.get_user(target_id)
    target_name = _escape_markdown(target_data.get('username', f'User {target_id}')) # Escaping target name
...
async def _check_joined(query: Update.callback_query, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for key, channel in channels.items():
        ...
        try:
            ...
            await query.message.reply_text(f"🚨 **VERIFICATION ERROR!** Bot is unable to verify membership for channel: `{channel_id}`. Please ensure the bot is an **Admin** there and try again. Error: {_escape_markdown(str(e))}", parse_mode='Markdown')
...
async def _send_content(context: ContextTypes.DEFAULT_TYPE, user_id: int, content_list: List[Dict[str, Any]], message_header: str) -> None:
    ...
    for content in content_list:
        try:
            ...
            error_message = _escape_markdown(str(e))
            await context.bot.send_message(chat_id=user_id, text=f"⚠️ **Content Error:** Failed to send a file. The bot might have lost access to the file. Error: `{error_message}`", parse_mode='Markdown')
...
async def _send_referral_prompt(query: Update.callback_query, context: ContextTypes.DEFAULT_TYPE, initial: bool = False) -> None:
    ...
    if not initial:
        # Determine the target reference count for the PREVIOUSLY defined tier to check for reward
        if next_tier_index > 0:
            reward_tier = TIERS[next_tier_index - 1]
            refs_needed_for_reward = reward_tier['refs_required']
            reward_count = reward_tier['reward_count']
        elif next_tier_index == 0 and current_refs >= TIERS[0]['refs_required']: 
             # Edge case: If current_refs >= 1 but it's the first tier
             reward_tier = TIERS[0]
             refs_needed_for_reward = reward_tier['refs_required']
             reward_count = reward_tier['reward_count']
        else:
            refs_needed_for_reward = -1 # No reward check needed yet

        
        # Check if the user has reached a new reward tier since the last recorded reward count
        if refs_needed_for_reward > 0 and current_refs >= refs_needed_for_reward and current_refs > user_data.get('last_reward_ref_count', 0):
            
            reward_content = DB.get_random_content(user_id, reward_count)
            
            if not reward_content:
                await context.bot.send_message(chat_id=user_id, text="🎉 You earned more content, but the library is currently empty or fully used. Check back soon!")
                user_data['last_reward_ref_count'] = current_refs # Mark as rewarded
                # Continue to send the prompt for the next tier
            else:
    ...
    try:
        ...
        logger.error(f"Failed to send referral prompt to user {user_id}. Error: {e}")
...
async def _handle_admin_user_action(query: Update.callback_query, context: ContextTypes.DEFAULT_TYPE, data: str) -> None:
    ...
    target_data = DB.get_user(target_id)
    target_name = _escape_markdown(target_data.get('username', f'User {target_id}'))
    ...
    if action == "toggleblock":
        ...
        # FIXED: Wrap the name and ID in backticks to prevent Markdown parsing issues
        await query.edit_message_text(
...
async def _handle_general_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if 'next_message_is_broadcast' in context.user_data and DB.is_authorized(user_id):
        ...
        try:
            ...
            error_message = _escape_markdown(str(e))
            await update.message.reply_text(f"❌ **BROADCAST FAILED!** Could not send message to user `{target_id}`. Error: `{error_message}`")
        return
