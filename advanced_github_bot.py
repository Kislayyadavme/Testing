...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if context.args and user_id != ADMIN_CHAT_ID: # Prevent admin self-referral
        try:
            ...
            if DB.add_referral(referrer_id, user_id):
                ...
                # Notify the referrer - FIXED: Wrap the username in backticks to prevent Markdown parsing issues (e.g., if username has underscores)
            ...
            # Log the error, but continue the start command flow for the new user
    ...
    # FIXED: The failure happened here when attempting to parse the combined text.
    # By fixing the referrer notification above, and ensuring dynamic parts are safe, this should work.
    # We will use parse_mode='Markdown' which is usually more forgiving than MarkdownV2 for simple formatting.
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # 4. Admin Management Flow
        ...
        target_name = DB.get_user(target_id).get('username', f'User {target_id}')
